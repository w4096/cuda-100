...
__global__ void kernel_wmma_smem(int M, int N, int K, float alpha, const half* A, const half* B, float beta, float* C){
    __shared__ half As[WAPRS_GRID_SIZE][WAPRS_GRID_SIZE][WMMA_TILE_SIZE][WMMA_TILE_SIZE];
    __shared__ half Bs[WAPRS_GRID_SIZE][WAPRS_GRID_SIZE][WMMA_TILE_SIZE][WMMA_TILE_SIZE];
    ...
    for(int k = 0; k < K; k += BLOCK_K){
        ...
        for (int i = 0; i < WMMA_TILE_SIZE * WMMA_TILE_SIZE; i += warpSize) {
            ...
            if (global_a_row < M && global_a_col < K) {
                As[warp_y][warp_x][a_row][a_col] = A[global_a_row * lda + global_a_col];
            } else {
                As[warp_y][warp_x][a_row][a_col] = half(0.0f);
            }
            ...
            if (global_b_row < K && global_b_col < N) {
                Bs[warp_y][warp_x][b_row][b_col] = B[global_b_row * ldb + global_b_col];
            } else {
                Bs[warp_y][warp_x][b_row][b_col] = half(0.0f);
            }
        }
        ...
        for (int i = 0; i < WAPRS_GRID_SIZE; i++) {
            const half* A_ptr = &As[warp_y][i][0][0];
            const half* B_ptr = &Bs[i][warp_x][0][0];
...
extern "C" void gemm_wmma_16x16x16_smem(int M, int N, int K, float alpha, const half* A, const half* B, float beta, float* C) {
    ...
    kernel_wmma_smem<WMMA_TILE_SIZE, WAPRS_GRID_SIZE><<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
